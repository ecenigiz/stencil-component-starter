...
const patchEsm = () => {
    ...
    if (BUILD.cssVarShim && !(win.CSS && win.CSS.supports && win.CSS.supports('color', 'var(--c)'))) {
        // @ts-ignore
        return __sc_import_stencil_starter_project_name('./css-shim-978387b1-1e75855f.js').then(() => {
...
const patchBrowser = async () => {
    ...
    if (importMeta !== '') {
        ...
        if (!window.customElements) {
            // @ts-ignore
            await __sc_import_stencil_starter_project_name('./dom-96781eef-a2fb04dd.js');
        }
